				...
				<table border="0" align="center" cellpadding="0" cellspacing="0" class="main-table">
					...
					<tr>
						<td width="200" class="content-right">
        					<h1>${manageName.firstName }&nbsp${manageName.lastName }</h1><br />
       					</td>
      					<td class="content">
      						<c:forEach items="${manageBlog }" var="singleBlog">
      							<a name="${singleBlog.blog.blogId }"></a>
      								<h1>Topic:${singleBlog.blog.title }</h1>
      								<p>Post by:${manageName.firstName } &nbsp${manageName.lastName }</p>
      								<p>
      								...
      								<form action="Delete" method="post">
      										<div class="hide">
      											...
      											<input type="text" name = "manager" value="1" />
      										</div>
      									<button id="submit" type="submit">Delete</button>
      								</form>
      								<p>Coments:</p>
      								<c:forEach items="${singleBlog.comments }" var="comment">
      									<p class="comments">${comment.reviewer.firstName }&nbsp${comment.reviewer.lastName }:${comment.context }</p>
      									<form action="Delete" method="post">
      											<div class="hide">
