    		...
    		<td>
				<table border="0" align="center" cellpadding="0" cellspacing="0" class="main-table">
					...
					<tr>
      					<td class="content">
      						<div>
      							<button type="submit" id="submit"><a href="/Final/GetFollows?send=1">Write Message</a></button>
      						</div>
      						<c:forEach items="${messages }" var="message">
      							<a href="/Final/readMessage.jsp?id=${message.messageId }">${message.title }</a>
      							<p>from <a href="/Final/Visit?visitId=${message.name.id}">${message.name.firstName }&nbsp${message.name.lastName }</a></p>
      							<br />						
      								<form action="Delete" method="post">
    									<div class="hide">
     									<input type="text" name = "deleteMessageId" value="${message.messageId }" />
      									</div>
